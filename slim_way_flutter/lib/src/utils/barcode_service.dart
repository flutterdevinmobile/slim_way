...
class BarcodeService {
  static Future<BarcodeProduct?> getProductByBarcode(String barcode) async {
    print('DEBUG: Scanning Barcode: $barcode');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 1 || data['status'] == "found") {
          ...
          print('DEBUG: Product found: ${result.name}');
          return result;
        } else {
          print('DEBUG: Product not found in Open Food Facts database');
        }
      }
    } catch (e) {
      print('DEBUG: Barcode Service Error: $e');
    }
    return null;
  }
}
