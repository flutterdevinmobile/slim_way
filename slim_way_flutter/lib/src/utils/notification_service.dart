...
void notificationTapBackground(NotificationResponse notificationResponse) async {
  if (notificationResponse.actionId == 'drink_water') {
    ...
    if (sendPort != null) {
      sendPort.send(250);
      print('Notification Action (Background): Signal sent through bridge');
    }
  }
}
...
class NotificationService {
    ...
    await _notificationsPlugin.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse response) async {
        if (response.actionId == 'drink_water') {
          ...
          print('Notification Action (Foreground): Fast-track update emitted');
        }
      },
    );
  ...
  static Future<void> _scheduleNotificationForSlot(int id, DateTime slotTime, {required bool isFollowUp}) async {
    ...
    final AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
      ...
      isFollowUp ? 'Water Nagging' : 'Water Reminders',
      channelDescription: isFollowUp ? 'Urgent reminders if you missed the first one' : 'Regular reminders to drink water',
    ...
    await _notificationsPlugin.zonedSchedule(
      id: id,
      title: isFollowUp ? 'Suv ichish esingizdan chiqdimi? 💧' : 'Suv vaqti!',
      body: isFollowUp 
          ? 'Salomatligingiz uchun bitta stakan suv ichib, \'Ichdim\' tugmasini bosing!'
          : 'Kamida yana 250ml suv so\'rayapti tanangiz. Bitta bemalol ichvoring!',
